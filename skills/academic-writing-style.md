...
description: 学术写作风格控制技能。 Use for: 需要把草稿修正为更符合学术论文风格的表达时使用。
---
...
需要把草稿修正为更符合学术论文风格的表达时使用。

## Core Pattern

统一术语、降低口语化、增强客观性、提升逻辑衔接与论证密度。
...
|---|---|
| 典型输入 | 段落草稿、章节草稿、论证目标。 |
| 典型输出 | 风格修订建议、表达升级稿、术语一致性检查。 |
| 依赖关系 | 可与 revision-and-proofreading 联动。 |
| 当前备注 | 重点避免营销化、空泛化和明显 AI 痕迹表达。 |

## Implementation Process

先确认当前任务是否属于 **academic-writing-style** 的职责范围，再检查是否存在前置技能或门禁约束。如果存在更高优先级的前置要求，例如文献核验、格式校验或答辩风险评估，应先满足前置条件，再进入本技能的主任务处理。执行过程中必须坚持实践导向、真实数据导向与学校规范导向，不得为了追求表达完整而虚构信息。
...
> 调用示例：`academic-writing-style：请基于当前草稿执行对应任务，并严格遵循 thesis Skills 体系规则。`
