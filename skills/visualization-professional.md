...
description: 专业图表与图示规范化技能。 Use for: 生成研究框架图、流程图、机制图、分析图、结果图表时使用。
---
...
生成研究框架图、流程图、机制图、分析图、结果图表时使用。

## Core Pattern

先确认图表目的，再检查数据或逻辑来源，再输出满足学术规范的自解释图表。

## Quick Reference

| 项目 | 内容 |
|---|---|
| 典型输入 | 数据、图示需求、章节位置、学校规则。 |
| 典型输出 | 图表方案、命名、标题/图注建议、规范性检查结论。 |
| 依赖关系 | 可与 data-analysis-guide、formatting-compliance-zju 联动。 |
| 当前备注 | 重点要求是自解释、专业化、与学校图表规范一致。 |

## Implementation Process

先确认当前任务是否属于 **visualization-professional** 的职责范围，再检查是否存在前置技能或门禁约束。如果存在更高优先级的前置要求，例如文献核验、格式校验或答辩风险评估，应先满足前置条件，再进入本技能的主任务处理。执行过程中必须坚持实践导向、真实数据导向与学校规范导向，不得为了追求表达完整而虚构信息。
...
> 调用示例：`visualization-professional：请基于当前草稿执行对应任务，并严格遵循 thesis Skills 体系规则。`
