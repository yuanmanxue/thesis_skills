...
description: 论文阶段进度跟踪技能。 Use for: 需要规划选题、开题、中期、预答辩、盲审、答辩等阶段任务时使用。
---
...
需要规划选题、开题、中期、预答辩、盲审、答辩等阶段任务时使用。

## Core Pattern

对照阶段目标盘点已完成、未完成、风险项、下一步安排。

## Quick Reference

| 项目 | 内容 |
|---|---|
| 典型输入 | 当前阶段、待办、已完成事项、时间节点。 |
| 典型输出 | 进度表、优先级建议、风险提示。 |
| 依赖关系 | 可与 thesis-orchestrator、defense-prep 联动。 |
| 当前备注 | 适合用于中期前后任务闭环管理。 |

## Implementation Process

先确认当前任务是否属于 **progress-tracker** 的职责范围，再检查是否存在前置技能或门禁约束。如果存在更高优先级的前置要求，例如文献核验、格式校验或答辩风险评估，应先满足前置条件，再进入本技能的主任务处理。执行过程中必须坚持实践导向、真实数据导向与学校规范导向，不得为了追求表达完整而虚构信息。
...
> 调用示例：`progress-tracker：请基于当前草稿执行对应任务，并严格遵循 thesis Skills 体系规则。`
